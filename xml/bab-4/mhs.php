<?php
	...
	$path = ($extension=='php') ? NULL : $_SERVER ['PATH INFO'];
	...
	if ($num_rows == 0){
		...
		$query = sprintf($query,$_POST['nim'],$_POST['alamat'],$_POST['prodi']);
	}else if ($num_rows == 1){
		$query = "UPDATE mahasiswa SET nim = '%s',nama = '%s',alamat = '%s',prodi = '%s' WHERE nim = '%s'";
		$query = sprintf($query,$_POST['nim'],$_POST['alamat'],$_POST['prodi'],$_POST['nim']);
	}
		$result = mysql_query($query) or die ('Query failed: '.mysql_error());
	}
